...
{
    public class SurveyReminderProcess
    {
        public static void StartProcess()
        {
            try
            {
                List<ProveedoresOnLine.SurveyModule.Models.SurveyModel> lstRemindersToSend =
                    ProveedoresOnLine.SurveyModule.Controller.SurveyModule.BPSurveyGetReminders();

                if (lstRemindersToSend != null && lstRemindersToSend.Count > 0)
                {
                    //log file
                    LogFile("Start send " + lstRemindersToSend.Count.ToString());

                    lstRemindersToSend.All(sv =>
                    {
                        try
                        {
                            if (sv.SurveyInfo != null)
                            {
                                MessageModule.Client.Models.ClientMessageModel oMessageToUpsert = GetReminder(sv, sv.User);
                                    ...
                                    SurveyInfo = new List<Company.Models.Util.GenericItemInfoModel>()
                                    {
                                        new Company.Models.Util.GenericItemInfoModel()
                                        {
                                            ItemInfoId = sv.SurveyInfo.
                                                Where(svinf=>svinf.ItemInfoType.ItemId == 1204015).
                    ...
                    //log file
                    LogFile("End send " + lstRemindersToSend.Count.ToString());
                }
            }
            catch (Exception err)
            {
        ...
        private static MessageModule.Client.Models.ClientMessageModel GetReminder(ProveedoresOnLine.SurveyModule.Models.SurveyModel vSurvey, string EvaluatorRol)
        {
            //Create message object
            MessageModule.Client.Models.ClientMessageModel oReturn = new MessageModule.Client.Models.ClientMessageModel()
            {
                Agent = ProveedoresOnLine.SurveyBatch.Models.Constants.C_POL_SurveyReminder_Mail_Agent,
                User = vSurvey.SurveyInfo.
                    Where(svinf => svinf.ItemInfoType.ItemId == 1204002).
                    Select(svinf => svinf.Value).
                    DefaultIfEmpty("SystemUser").
        ...
        private static void LogFile(string LogMessage)
        {
            try
            {
                ...
                LogFile += "\\" + "Log_SurveyReminderProcess_" + DateTime.Now.ToString("yyyyMMdd") + ".log";
