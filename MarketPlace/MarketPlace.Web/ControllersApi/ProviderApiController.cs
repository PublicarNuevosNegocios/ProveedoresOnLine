...
namespace MarketPlace.Web.ControllersApi
{
    public class ProviderApiController : BaseApiController
    {
        ...
        [HttpPost]
        [HttpGet]
        public HttpResponseMessage ReportGeneralCompare(string ReportGeneralCompare, string SearchParam, string SearchFilter)
        {
            if (ReportGeneralCompare == "true")
            {
                    ...
                    !string.IsNullOrEmpty(SessionModel.CurrentCompany.CompanyPublicId))
                {
                    ...
                    result.Content.Headers.ContentDisposition.FileName = "Proveedores_" + DateTime.Now.ToString("yyyyMMddHHmm") + ".csv";
                    return result;
