...
namespace ProveedoresOnLine.CompanyProvider.DAL.MySQLDAO
{
    internal class CompanyProvider_MySqlDao : ICompanyProviderData
    {
        ...
        public ProveedoresOnLine.CompanyProvider.Models.Integration.CustomDataModel CustomerProvider_GetField(ProveedoresOnLine.Company.Models.Company.CompanyModel Customer, string ProviderPublicId)
        {
            ...
            {
                CommandExecutionType = ADO.Models.enumCommandExecutionType.DataSet,
                CommandText = "IP_CustomerRedirect_GetAllField",
                CommandType = CommandType.StoredProcedure,
                Parameters = lstParams,
            });

            ProveedoresOnLine.CompanyProvider.Models.Integration.CustomDataModel oReturn = new Models.Integration.CustomDataModel();

            if (response.DataSetResult.Tables[0] != null &&
                response.DataSetResult.Tables[0].Rows.Count > 0)
            {
                oReturn.AditionalField =
            ...
            oReturn.RelatedCustomer = Customer;
