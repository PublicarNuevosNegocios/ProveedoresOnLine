...
namespace ProveedoresOnLine.CompanyProvider.Controller
{
    public class CompanyProvider
    {
        ...
        public static ProveedoresOnLine.CompanyProvider.Models.Integration.CustomDataModel CustomerProvider_CustomData_Upsert(ProveedoresOnLine.CompanyProvider.Models.Integration.CustomDataModel CustomData, string ProviderPublicId)
        {
            string RelatedCustomer = CustomData.RelatedCustomer.CompanyPublicId;

            switch (RelatedCustomer)
            {
                case "7BC27832": //SANOFI
                    if (CustomData.AditionalData != null &&
                        CustomData.AditionalData.Count > 0)
                    {
                        CustomData.AditionalData.All(dt =>
                        {
                            LogManager.Models.LogModel oLog = Company.Controller.Company.GetGenericLogModel();
                            try
                            {
                                dt.ItemId = DAL.Controller.CompanyProviderDataController.Instance.Sanofi_AditionalData_Upsert(
                                    dt.ItemId,
                                    ProviderPublicId,
        ...
        public static GenericItemModel CustomerProvider_CustomDataInfo_Upsert(GenericItemModel oCustomDataToUpsert)
        {
                ...
                oCustomDataToUpsert.ItemInfo.Count > 0)
            {
                oCustomDataToUpsert.ItemInfo.All(dti =>
                {
                    ...
                    try
                    {
                        dti.ItemInfoId = DAL.Controller.CompanyProviderDataController.Instance.Sanofi_AditionalDataInfo_Upsert(
                            dti.ItemInfoId,
